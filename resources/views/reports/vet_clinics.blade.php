...
@section('title', 'Veterinary Clinics')

@section('header', 'Veterinary Clinics Registry')
@section('subheader', 'Registered veterinary clinics and hospitals')
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-800 mt-1">{{ \App\Models\Establishment::where('type', 'vet_clinic')->count() }}</p>
            </div>
            <div class="w-14 h-14 bg-teal-100 rounded-xl flex items-center justify-center">
                <i class="bi bi-hospital text-teal-600 text-2xl"></i>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-green-600 mt-1">{{ \App\Models\Establishment::where('type', 'vet_clinic')->where('status', 'active')->count() }}</p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-red-600 mt-1">{{ \App\Models\Establishment::where('type', 'vet_clinic')->where('status', 'inactive')->count() }}</p>
            </div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-8">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <a href="#" class="flex flex-col items-center p-4 bg-teal-50 hover:bg-teal-100 rounded-xl transition group">
            <div class="w-12 h-12 bg-teal-600 rounded-xl flex items-center justify-center mb-2 group-hover:scale-110 transition">
                <i class="bi bi-plus-circle text-white text-xl"></i>
            </div>
            <span class="text-sm font-medium text-gray-700">Add Clinic</span>
        </a>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
        <h3 class="text-lg font-semibold text-gray-800">Registered Veterinary Clinics</h3>
        ...
        <table class="w-full">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Owner/Operator</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse(\App\Models\Establishment::where('type', 'vet_clinic')->get() as $clinic)
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">License: {{ $clinic->license_number }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $clinic->owner_name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $clinic->address }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $clinic->contact_number }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-8 text-center text-gray-500">
                            <i class="bi bi-hospital text-4xl mb-2 block"></i>
                            No veterinary clinics registered yet
                        </td>
                    </tr>
                @endforelse
            </tbody>
