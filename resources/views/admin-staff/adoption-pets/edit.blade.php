...
<div class="p-4 md:p-6">
    <div class="flex items-center gap-4 mb-6">
        <a href="{{ route('admin-staff.adoption-pets.show', $pet) }}" class="p-2 hover:bg-gray-100 rounded-lg transition">
            <i class="bi bi-arrow-left text-gray-600"></i>
        </a>
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Edit {{ $pet->pet_name }}</h1>
            <p class="text-gray-500 mt-1">Update pet information</p>
        </div>
    </div>

    <div class="bg-white rounded-xl shadow-sm">
        <form method="POST" action="{{ route('admin-staff.adoption-pets.update', $pet) }}" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="pet_name" id="pet_name" value="{{ old('pet_name', $pet->pet_name) }}" required
                ...
                <div>
                    ...
                    <input type="date" name="date_of_birth" id="date_of_birth" value="{{ old('date_of_birth', $pet->date_of_birth ? $pet->date_of_birth->format('Y-m-d') : '') }}"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">
                </div>

                <div>
                    <label for="weight" class="block text-sm font-medium text-gray-700 mb-1">Weight (kg)</label>
                    <input type="number" name="weight" id="weight" value="{{ old('weight', $pet->weight) }}" step="0.1" min="0"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">
                </div>
                ...
                <div class="md:col-span-2">
                    <div class="flex items-center gap-2 mb-2">
                        <input type="checkbox" name="is_age_estimated" id="is_age_estimated" value="1" 
                            {{ old('is_age_estimated', $pet->is_age_estimated) ? 'checked' : '' }}
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">{{ old('description', $pet->description) }}</textarea>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    @if($pet->image)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $pet->image) }}" alt="Current photo" class="w-32 h-32 object-cover rounded-lg">
            ...
            <div class="flex items-center justify-end gap-4 mt-6 pt-4 border-t border-gray-100">
                <a href="{{ route('admin-staff.adoption-pets.show', $pet) }}" class="px-4 py-2 text-gray-600 hover:text-gray-800 hover:bg-gray-100 rounded-lg transition">
                    Cancel
                </a>
