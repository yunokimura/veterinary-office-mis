...
@section('title', 'Stock Out - ' . $item->item_name)
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
            <h3 class="text-lg font-semibold text-gray-800">Stock Out - {{ $item->item_name }}</h3>
            <p class="text-sm text-gray-500">Current Stock: {{ $item->quantity }} {{ $item->unit }}</p>
        </div>

        <form action="{{ route('inventory.stock-out.process', $item) }}" method="POST" class="p-6">
            @csrf

            <div class="mb-6">
                <label for="quantity" class="block text-sm font-medium text-gray-700 mb-2">Quantity <span class="text-red-500">*</span></label>
                <input type="number" name="quantity" id="quantity" value="{{ old('quantity') }}" min="1" max="{{ $item->quantity }}" 
            ...
            <div class="mb-6">
                ...
                <textarea name="remarks" id="remarks" rows="3" 
                    class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-red-500 focus:border-red-500 transition"
                    placeholder="Reason for stock out">{{ old('remarks') }}</textarea>
            </div>
            ...
            <div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                <p class="text-sm text-red-600">New Stock Level</p>
                <p class="text-2xl font-bold text-red-700">{{ $item->quantity }} - <span id="preview-quantity">0</span> = <span id="preview-total">{{ $item->quantity }}</span> {{ $item->unit }}</p>
            </div>
            ...
            <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-100">
                ...
                <button type="submit" class="px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg transition shadow-sm">
                    <i class="bi bi-dash-circle mr-2"></i>Remove Stock
                </button>
            </div>
        </form>
    </div>
...
document.getElementById('quantity').addEventListener('input', function() {
    ...
    document.getElementById('preview-total').textContent = Math.max(0, currentStock - quantity);
});
</script>
@endsection
